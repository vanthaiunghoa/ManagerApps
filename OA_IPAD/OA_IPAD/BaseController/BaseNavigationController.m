...
//    [navBar setBackgroundImage:[UIImage imageNamed:navBarBg] forBarMetrics:UIBarMetricsDefault];
    ...
    navBar.barTintColor = [UIColor colorWithHex:0x3D98FF];
    // 返回按钮颜色
    navBar.tintColor = [UIColor whiteColor];
   
    [navBar setTitleTextAttributes:@{NSFontAttributeName:[UIFont boldSystemFontOfSize:22], NSForegroundColorAttributeName:[UIColor whiteColor]}];
//    navBar
...
-(void)pushViewController:(UIViewController *)viewController animated:(BOOL)animated
{
    if (self.childViewControllers.count > 0) { // 如果push进来的不是第一个控制器
        ...
        [btn setImage:[UIImage imageNamed:@"arrow-back"] forState:UIControlStateNormal];
