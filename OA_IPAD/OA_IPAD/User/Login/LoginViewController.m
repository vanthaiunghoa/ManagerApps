...
- (void)loadView
{
    ...
    [_loginView reloadData:self.viewModel.account AndPassword:self.viewModel.password];
}
...
//            self.isYetVPNLoginSuccess = NO;
        ...
        [_loginView reloadData:@"" AndPassword:@""];
    }
...
- (void)didLoginWithUserName:(NSString *)username AndPassWord:(NSString *)password
{
    ...
    {
        self.viewModel.account = username;
...
-(void)login
{
    ...
    // 发送登录请求； RACCommand内部已经做了去重处理，不会发多个请求
    @weakify(self);
    [[self.viewModel.loginCommand execute:nil] subscribeNext:^(id  _Nullable x) {
        @strongify(self);
        self.view.userInteractionEnabled = YES;
...
//        userModel.password = self.password;
        ...
        [self.navigationController pushViewController:[NSClassFromString(@"MainViewController") new] animated:YES];
        ...
        self.viewModel.loginCommand = nil;
        PLog(@"登录失败: %@", error);
    }];
}
