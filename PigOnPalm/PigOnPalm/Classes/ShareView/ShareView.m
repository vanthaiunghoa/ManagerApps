...
- (id)initWithTitleArray:(NSMutableArray *)titlearray picarray:(NSMutableArray *)picarray
{
    ...
    if (self) {
        ...
        UILabel *labTitle = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, SCREENWIDTH, 44)];
        labTitle.text = @"分享至";
        labTitle.textAlignment = NSTextAlignmentCenter;
        labTitle.userInteractionEnabled = YES;
        [self.largeView addSubview:labTitle];
        ...
        UIScrollView *scroll = [[UIScrollView alloc]initWithFrame:CGRectMake(0, 44, SCREENWIDTH, 90)];
...
//        [_largeView addSubview:pageShow];
        ...
        for (int i = 0; i < titlearray.count; i ++) {
            SingleView *rr = [[SingleView alloc]initWithFrame:CGRectMake(i *(SCREENWIDTH / 4), 0, SCREENWIDTH/4, 90)];
            rr.tag = 10 + i;
            rr.sheetBtn.tag = i;
//            rr.sheetBtn.backgroundColor = [UIColor redColor];
...
//            [rr.sheetBtn setImage:[UIImage imageNamed:[NSString stringWithFormat:@"%@",picarray[i]]] forState:UIControlStateNormal];
            [rr.imageView setImage:[UIImage imageNamed:[NSString stringWithFormat:@"%@",picarray[i]]]];
...
//
//            SingleView *rr =  (SingleView *)[self viewWithTag:10 + i];
