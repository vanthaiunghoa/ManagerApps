...
- (NSString *)getReceiveCommonModelPath
{
    ...
    NSString *cachesPath = NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES)[0];
    // 拼接文件名
    NSString *filePath = [cachesPath stringByAppendingPathComponent:@"ReceiveCommonModel.data"];
    return filePath;
}

- (void)saveReceiveCommonModel:(ReceiveCommonModel *)model
{
    [NSKeyedArchiver archiveRootObject:model toFile:[self getReceiveCommonModelPath]];
}

- (ReceiveCommonModel *)getReceiveCommonModel
{
    return [NSKeyedUnarchiver unarchiveObjectWithFile:[self getReceiveCommonModelPath]];
}

- (void)deleteReceiveCommonModel
{
    NSFileManager *defaultManager = [NSFileManager defaultManager];
    if ([defaultManager isDeletableFileAtPath:[self getReceiveCommonModelPath]])
    {
        [defaultManager removeItemAtPath:[self getReceiveCommonModelPath] error:nil];
    }
}
