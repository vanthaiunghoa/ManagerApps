...
- (void)setModel:(IconLabelModel *)model {
    _textLabel.text = model.text;
    _iconView.image = model.icon;
}

- (void)updateLayoutBySize:(CGSize)size finished:(void (^)(IconLabel *))finished {
    CGRect frame = self.frame;
    frame.size = size;
    self.frame = frame;
...
@implementation IconLabelModel

+ (instancetype)modelWithTitle:(NSString *)title image:(UIImage *)image {
    IconLabelModel *model = [IconLabelModel new];
    model.text = title;
    model.icon = image;
    return model;
}
