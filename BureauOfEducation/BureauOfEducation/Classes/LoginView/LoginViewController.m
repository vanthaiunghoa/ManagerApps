...
@interface LoginViewController ()<LoginViewDelegate, SKPSMTPMessageDelegate, SangforAuthDelegate>
...
- (void)L3VPNLogin
{
    NSURL *vpnUrl = [NSURL URLWithString:@"https://jyj.dgjy.net"];
    [_sdkManager startPasswordAuthLogin:_mode vpnAddress:vpnUrl
                               username:@"oatest" password:@"dgjy.123"];
}
...
- (void)onLoginFailed:(NSError *)error
{
    ...
    {
        [SVProgressHUD showErrorWithStatus:[NSString stringWithFormat:@"%@,code=%ld",error.domain,(long)error.code]];
    }
}
...
{
//    [SVProgressHUD showInfoWithStatus:@"认证过程回调"];
//    switch (nextAuthType) {
